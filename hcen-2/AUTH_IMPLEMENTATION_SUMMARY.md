...
**refresh_tokens Document Schema**:
```json
{
  "_id": ObjectId("..."),
  "tokenHash": "sha256:...",
...
**Indexes**:
- `userCi`, `tokenHash` (unique), `expiresAt`, `isRevoked`
...
- WildFly configuration (HTTPS, CORS)
- MongoDB setup and tuning
...
- MongoDB (refresh token storage)

#### 3. RefreshToken Document and Repository

**Location**:
- `uy.gub.hcen.auth.document.RefreshToken`
...
public class RefreshTokenRepository {
    ...
    public void save(RefreshToken token) {
        MongoCollection<Document> collection = mongoDb.getCollection("refresh_tokens");
        collection.insertOne(toDocument(token));
    }

    public Optional<RefreshToken> findByTokenHash(String hash) {
        MongoCollection<Document> collection = mongoDb.getCollection("refresh_tokens");
        Document doc = collection.find(eq("tokenHash", hash)).first();
        return Optional.ofNullable(doc).map(this::fromDocument);
    }

    public List<RefreshToken> findByUserCi(String ci) {
        MongoCollection<Document> collection = mongoDb.getCollection("refresh_tokens");
        return collection.find(eq("userCi", ci))
    ...
    public void revokeAllForUser(String ci) {
        ...
        collection.updateMany(
            eq("userCi", ci),
            combine(set("isRevoked", true), set("revokedAt", new Date()))
        );
    }

    public void deleteExpired() {
        MongoCollection<Document> collection = mongoDb.getCollection("refresh_tokens");
        collection.deleteMany(lt("expiresAt", new Date()));
    }
}
```
...
4. **MongoDB Indexes**
   - Index refresh_tokens.tokenHash (unique queries)
   - Index refresh_tokens.userCi (user lookup)
   - Index refresh_tokens.expiresAt (cleanup job, TTL index)
