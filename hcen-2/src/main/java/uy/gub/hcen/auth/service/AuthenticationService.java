...
public class AuthenticationService {
    ...
    public LoginInitiateResponse initiateLogin(LoginInitiateRequest request) {
        try {
            ...
            String state = request.getState() != null ? request.getState() : StateUtil.generateState();

            // Store state in Redis with metadata
            stateManager.saveState(state, request.getClientType(), request.getRedirectUri(),
    ...
    public TokenResponse handleCallback(CallbackRequest request) {
        try {
            ...
            // 6. Generate HCEN JWT tokens
            String accessToken = jwtService.generateAccessToken(userCi, userInfo.getInusId(), "PATIENT");
            ...
            // 9. Return response
            return new TokenResponse(accessToken, refreshToken, "Bearer", 3600, userInfo);
    ...
    public TokenResponse refreshToken(String refreshTokenValue) {
        try {
            ...
            String newAccessToken = jwtService.generateAccessToken(userCi, inusId, "PATIENT");
            ...
            return new TokenResponse(newAccessToken, newRefreshToken, "Bearer", 3600, userInfo);
