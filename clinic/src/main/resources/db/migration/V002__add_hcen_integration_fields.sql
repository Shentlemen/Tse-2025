...
-- Add comments
COMMENT ON COLUMN clinics.hcen_jms_url IS 'URL para conexion JMS con HCEN (ej: http-remoting://localhost:8080)';
COMMENT ON COLUMN clinics.api_key IS 'API key para autenticacion REST con HCEN';
...
-- Add comments
COMMENT ON TABLE access_requests IS 'Solicitudes de acceso a documentos externos de otras clinicas';
COMMENT ON COLUMN access_requests.patient_ci IS 'Cedula del paciente';
COMMENT ON COLUMN access_requests.document_id IS 'ID del documento especifico (opcional)';
