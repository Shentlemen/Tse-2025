 ...
 * HCEN Message Sender
 *
 * Stateless EJB for sending messages to HCEN Central JMS queues.
 *
 * Uses JMS 2.0 simplified API with CDI injection.
 * Changed from @Singleton to @Stateless to fix transaction context issues.
 *
 * @author TSE 2025 Group 9
 * @version 2.1
 * @since 2025-11-13
...
public class HcenMessageSender {
    ...
    // Use @Resource injection instead of @Inject to avoid remote naming issues
    // ConnectionFactory is used to create JMSContext per method call
    @Resource(lookup = "java:/ConnectionFactory")
    private ConnectionFactory connectionFactory;

    // Use @Resource to inject the configured queues.
    // Using local JNDI names (java:/) for same-server messaging.
    @Resource(lookup = "java:/jms/queue/UserRegistration")
    private Queue userRegistrationQueue;

    @Resource(lookup = "java:/jms/queue/DocumentRegistration")
     ...
     * @param clinicId Clinic identifier
     */
    public void sendPatientRegistration(
            String ci,
            String firstName,
            String lastName,
            java.time.LocalDate birthDate,
        ...
        try {
            ...
            // Create JMSContext and send message
            try (JMSContext context = connectionFactory.createContext()) {
                sendToQueue(context, userRegistrationQueue, message);
            }
     ...
     * @param documentLocatorUrl URL to retrieve the document from this prestador
     */
    public void sendDocumentMetadata(
            String patientCI,
            Long documentId,
            String documentType,
            String documentTitle,
        ...
        try {
            ...
            // Create JMSContext and send message
            try (JMSContext context = connectionFactory.createContext()) {
                sendToQueue(context, documentRegistrationQueue, message);
            }
    ...
    /**
     * Send message to JMS queue using simplified API.
     * The JMSContext handles connection, session, and producer creation.
     *
     * @param context The JMS context to use
     * @param queue The queue to send the message to
     * @param messageText JSON message text
     */
    private void sendToQueue(JMSContext context, Queue queue, String messageText) {
